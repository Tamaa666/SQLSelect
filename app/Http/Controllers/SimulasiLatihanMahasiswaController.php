...
use Auth;
use DB;
class SimulasiLatihanMahasiswaController extends Controller
{
    ...
    public function index()
    {
        ...
        {
            $materi[$value->id] = [];
            $materiSimulasi = DB::table('simulasi_materi')->where('type','latihan')->where('simulasi_id',$value->id)->get();
        ...
        return view('simulasi_latihan_mahasiswa.index',compact('data','materi','now','paket','pernah','soalPertama','nilai','percobaan'));
    }
    ...
    public function kerjakan($simulasi_id,$paket_id,$soal_id)
    {
        ...
        {
            return redirect('simulasi_latihan_mahasiswa')->with('error','Data pendukung pada simulasi ini tidak ditemukan atau tidak dapat digunakan silahkan hubungi dosen anda');
        }
        $petunjuk = json_decode(json_encode($petunjuk),true);
        $petunjukKolom = array_keys($petunjuk[0]);
        return view('simulasi_latihan_mahasiswa.kerjakan',compact('simulasi','paket','soal','now','soalBerikutnya','selesai','soalKe','petunjukKolom','petunjuk'));
    }
    ...
    public function selesai($simulasi_id,$type,$sesi)
    {
       ...
       if($simulasi)
       {
        $log = DB::table('simulasi_log')
           ...
           if($type == 'selesai')
           {
                return redirect('simulasi_latihan_mahasiswa')->with('success','Simulai sudah selesai dikerjakan');
           }
       }else
       {
           return redirect()->back()->with('error','data simulasi tidak ditemukan');
    ...
    public function lanjut(Request $request)
    {
        ...
        {   
            ...
            if(!is_array($sqlJawaban))
            {
                ...
                if($request->button != 'jalankan')
                {
                    if($simulasi->skip)
                    {
                        ...
                        return redirect('simulasi_latihan_kerjakan'.'/'.$request->simulasi_id.'/'.$request->paket_id.'/'.$request->soal_id);
                    }
                }
                if($simulasi->skip)
                {
            ...
            }else
            {
                ...
                {
                    return redirect('simulasi_latihan_mahasiswa')->with('error','data soal tidak ditemukan');
                }
            }
        }
    }
    ...
    public function uploadFile(Request $request, $oke)
    {
        ...
        $key = rand() . '_' . $request->simulasi_id.'_latihan_'.$request->soal_id.'_'.$mhs->id.'_log';
        $tmp_file_name = "{$key}.{$extension}";
        $tmp_file_path = "asset_application/jawaban/latihan";
